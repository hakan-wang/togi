...
final class VoiceCommandAgent {
    ...
    static func systemPrompt(now: Date) -> String {
        ...
        Rules:
        - Use "schedule" ONLY when you have a clear title AND a concrete start time. If the user \
        did not say how long it lasts, default the end to 30 minutes after the start. "say" \
        confirms it naturally, e.g. "done, i've put a call with sarah on your calendar for \
        tomorrow at 3 pm."
