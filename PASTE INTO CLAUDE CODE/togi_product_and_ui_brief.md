...
- **Real** — what you actually did. Backward-looking. The past. The recorded reality, categorized and color-coded.

> **Naming note:** the name is **Real** (the two toggle labels are **Plan / Real**). This brief uses *Real* throughout.

**The open UI decision (build variants to compare):**
- **Variant A — Two calendars side by side or as separate tabs.** Plan on one, Real on the other. Clearest separation, but you lose the at-a-glance comparison.
- **Variant B — One calendar with a Plan / Real toggle.** You look at the same day and flip between intention and reality. Strongest for *feeling* the gap, which is the emotional core of the product.
- **Variant C — One calendar, both overlaid.** Each block shows plan vs real together (e.g., the planned block with the real outcome layered or diffed on top). Most powerful but hardest to design without clutter.
...
### 6.1 The Plan / Real calendar view
The heart of the app. Shows intention (Plan) and recorded reality (Real). Blocks are categorized and color-coded so a glance tells you the shape of your day.
...
- Voice input is started by tapping the axolotl or a hotkey. "Hey Togi" as a wake phrase is a future goal, not yet built.
...
Every input, both plans and check-ins, gets automatically categorized using four independent fields:
- **domain** — fixed enum of 7; the AI never invents a new one: Work, Study, Health, Social, Errands & life admin, Leisure, Distraction. Drives colors and top-level analytics. Distraction is deliberately separate from Leisure (leaked time vs chosen rest).
- **project** — created only when the user explicitly declares one; the AI never invents a project; nullable.
- **activity** — a short reusable verb phrase; the AI may freely create a new one when nothing in the existing list fits.
- **note** — optional one-liner, max ~12 words, only if it adds information beyond the other three fields; never the raw transcript.
...
### 6.8 Behavioral learning for future planning
Togi learns from your history. If you've repeatedly failed to realize a 3-hour editing plan, Togi takes that into account in the next planning session and helps you set a plan you'll actually hit. Built on the short-term behavior database (habits/patterns) plus the long-term life database (context).
...
- Togi present and reachable by voice (tap the axolotl or a hotkey), front and center.
- The Plan / Real calendar view (intention + recorded reality). This is the centerpiece; prototype Variants A, B, and C from section 5.
...
**Tabs will be necessary** — not everything fits on one screen. Suggested structure: Main (Togi + Plan/Real calendar + sessions), Plan, Real, Data/Insights, Settings.
