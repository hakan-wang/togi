...
> - **Categorization:** 4-field model (domain / project / activity / note). Brief §6.5 and code now match.
...
| Deploy (web app) | **MISSING** | — | Web app not deployed yet; runs locally. Landing (heytogi.com) deploys via Cloudflare Pages. |
...
6. **Deploy the web app** (e.g. Cloudflare Pages or Vercel) and point a domain. Add production URLs to Supabase auth + Google OAuth.
